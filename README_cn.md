...
- **Agent 沙盒**：`aria-sandbox` 插件支持三种可切换后端——`docker`（默认）/ `kata` / `cubesandbox`（[CubeSandbox](https://github.com/TencentCloud/CubeSandbox)，E2B 兼容）；每个 agent 拥有一个隔离的持久化工作空间。
...
│                    workspace_status（docker / kata / cubesandbox 三种后端，默认 docker）
...
- 沙盒后端：默认 `docker`（需可用的 `docker`/`nerdctl CLI）；`kata` 同理（VM 隔离容器）；`cubesandbox` 需 x86_64 Linux + KVM（`/dev/kvm`），见 [CubeSandbox](https://github.com/TencentCloud/CubeSandbox)。
...
选择 provider 路由 `aria`。沙盒后端由 `ARIA_SANDBOX_TYPE` 选择（默认 `docker`；可选 `kata` / `cubesandbox`）。cubesandbox 的模板 id 由 `CUBE_TEMPLATE_ID` 读取（env 或 `.env`）；未设置时 `sandbox_*` 工具会明确报错。docker/kata 用 `ARIA_SANDBOX_IMAGE` / `ARIA_SANDBOX_CLI` / `ARIA_SANDBOX_RUNTIME` 配置镜像、CLI 与运行时。
...
- 隔离三层：cubesandbox 每工作空间独立 KVM MicroVM；docker/kata 每工作空间独立容器（kata 为 VM 隔离）+ 独立 0700 宿主目录 + 工具层路径校验（仅 `/workspace`，拒绝 `..` 逃逸）。
- 持久化：`sandbox_sync_to_host` / `sandbox_sync_from_host`（docker/kata 因卷挂载实时同步故为 no-op，仅 cubesandbox 生效）；可选 `ARIA_WORKSPACE_SYNC_AFTER_EXEC=true` 每次 `sandbox_exec` 后自动拉回（cubesandbox）。

## 配置

完整环境变量表见 `requirements.md` §2（`ARIA_MODEL_BUNDLE`、`ARIA_FFI_LIB`、`ARIA_ENGINE_MODEL`、`ARIA_MEMO_*`、`ARIA_SANDBOX_TYPE`、`ARIA_SANDBOX_IMAGE`、`ARIA_SANDBOX_CLI`、`ARIA_SANDBOX_RUNTIME`、`E2B_API_URL`、`E2B_API_KEY`、`CUBE_TEMPLATE_ID`、`E2B_TIMEOUT_MS`、`ARIA_WORKSPACE_ROOT`、`ARIA_WORKSPACE_SYNC_AFTER_EXEC`、`ARIA_WORKSPACE_ID`、`ARIA_REEF_*`）。
